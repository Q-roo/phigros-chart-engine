...
public class UnsafeVM(ByteCodeChunk chunk) {
    ...
    private readonly Stack<ICBValue> stack = new(200);
    ...
    private void ExecuteInstruction(UnsafeOpCode opCode) {
        switch (opCode) {
            ...
            case UnsafeOpCode.DCLV:
                ...
                chunkInfo.GetVariable(ReadAddress()).SetValueUnsafe(new NullValue());
                break;
            case UnsafeOpCode.ASGN: { // a
                // b
                // asgn
                ICBValue b = stack.Pop();
                stack.Pop().SetValue(b);
                break;
            }
            case UnsafeOpCode.DSPA:
                // TODO: new value type
                stack.Push(new I32Value(ReadAddress()));
                break;
            case UnsafeOpCode.DSPI:
                stack.Push(new I32Value(ReadI32()));
                break;
            case UnsafeOpCode.DSPD:
                stack.Push(new F32Value(ReadF32()));
                break;
            case UnsafeOpCode.DSPB:
                stack.Push(new BoolValue(ReadBool()));
                break;
            ...
            case UnsafeOpCode.ACOL:
                ...
                values.Reverse();
                stack.Push(new ArrayValue() { values = values });
                break;
            ...
            case UnsafeOpCode.BINOP: {
                ...
                ICBValue b = stack.Pop();
                stack.Push(stack.Pop().ExecuteBinaryOperatorUnsafe(@operator, b));
                break;
            }
            ...
            case UnsafeOpCode.MGET: {
                ICBValue b = stack.Pop();
                stack.Push(stack.Pop().GetMemberUnsafe(b));
                break;
            }
            case UnsafeOpCode.JMP: {
                programCounter = ((I32Value)stack.Pop()).value;
                break;
            }
            case UnsafeOpCode.JMPI: {
                int address = ((I32Value)stack.Pop()).value;
                BoolValue condition = new BoolType().Constructor(stack.Pop()).Case switch {
                    BoolValue v => v,
                    _ => throw new UnreachableException()
                };
